...
namespace RichEditAPISample.CodeExamples
{
    class TableActions
    {
        ...
        static void CreateFixedTable(Document document)
        {
            ...
            table.MergeCells(table[2, 1], table[3, 1]);
        ...
        static void MergeCells(Document document) {
            ...
            table.ForEachCell(delegate (TableCell cell, int i, int j) {
                document.AppendText(cell.VerticalMerging.ToString() + " ");
                if (cell == cell.Row.LastCell) document.Paragraphs.Append();
            });
            table.EndUpdate();
